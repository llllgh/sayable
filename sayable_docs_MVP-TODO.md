...
> **MVP 边界**：语音用于可编辑转写和朗读，不把文本匹配、语速、停顿或词时长启发式包装成口语能力评分。发音评测只有在未来接入可靠的专项服务后才重新立项。
...
- [x] **T8.6** MVP 语音转写：
  - partial / final 文本直接进入可编辑输入框，提交前由用户核对。
  - 录音时长、词级时间戳等服务事实可供链路诊断，但不生成能力分数或口语质量结论。
  - 原始音频和词级时间戳不进入数据库或自动备份。
- [ ] **T8.7** 时延与降级：停止说话到最终结果 P95 目标 ≤3 秒；LLM 只解释结构化指标，不进入原始评分关键链路。任何云端错误都必须保留用户输入并允许切回系统语音。

**完成判据**：首启双 Key 和三项能力验证不可跳过；系统/云端模式可切换；中国大陆/海外 Profile 可独立测试；已完成首启后云端临时不可用时仍能输入和朗读；真实一句英文结束后约 3 秒内返回可编辑转写。
...
| W5 | T8（语音双模式 + 区域 Profile + 可编辑转写） | 国内外网络可切换，云端失败可降级 |
