...
> 目标：装一次、选择中国大陆或海外服务区域并填写必要凭证、**数据不会因为清缓存或隐身模式丢失**、离线能用、能主动找到你。
...
- [ ] **T2.1** **Provider 抽象**：业务只调 `llm.capture() / llm.judge() / llm.compress() / llm.preflight()`，不知道协议差异。MVP 由 `serviceRegion ∈ cn | global` 选择内置 Profile；Profile 固定协议和 Base URL，模型使用厂商公共模型名。保留高级覆盖能力，但不暴露在普通流程。
- [x] **T2.2** **首启引导只有一屏**：选择「中国大陆 / 海外」，分别填写空白的模型 API Key 与语音 API Key；依次验证文本模型、ASR 鉴权和 TTS，全部成功后才允许进入。默认模型名和 Base URL 由 Profile 提供，不显示为已使用过的表单内容。
...
> **MVP 边界**：提供稳定、低时延的「可懂度 / 完整度 / 流利度 / 节奏」反馈，不宣称音素级发音评测。精确音素、重音和语调评分由长期后端接入专用 Pronunciation Assessment 服务实现。

- [x] **T8.1** 两个正交设置：
  - `voiceMode ∈ system | cloud`：系统语音零配置、云端增强使用大模型 ASR/TTS。
  - `serviceRegion ∈ cn | global`：分别选择中国大陆和海外 Profile；系统语音模式不依赖区域。
- [x] **T8.2** 内置 Profile Catalog，不要求填写 Base URL：
  - `cn`：火山引擎公共模型名 + 流式 ASR + 豆包 TTS 2.0。
  - `global`：BytePlus 公共模型名 + 流式 ASR + Seed Speech TTS。
  - 每个 Profile 独立保存模型名、协议、Base URL、Resource ID 和能力开关；不同区域不得假设模型名完全相同。
- [x] **T8.3** 凭证分能力写入 Keystore：LLM 与 Speech 使用独立 Key 槽，且按区域隔离；同一 Profile 的 ASR/TTS 在厂商允许时共用 Speech Key。凭证不进入 SQLite、Capacitor 日志、导出和备份。MVP 允许唯一使用者手动配置。
- [x] **T8.4** 云端 TTS：通过 SSE 接收 MP3 分片，MVP 在完整短句到达后播放，并按 `profile + voice + rate + text hash` 缓存；失败、离线或未配置时无缝降级 Android 系统 TTS。边接收边播放留作真实时延基线后的优化。
- [x] **T8.5** 云端 ASR：原生录音统一为 16 kHz、16-bit、单声道 PCM，边录边发送，返回 partial / final 文本与词级时间戳；失败时回到系统输入法语音路径。海外真实凭证的 WebSocket 鉴权及合成 PCM partial/final 转写已通过，真机麦克风输入仍需发布前复验。
- [x] **T8.6** MVP 语音反馈：
  - 可懂度：识别文本与目标文本的归一化差异。
  - 完整度：漏读、增读和目标词覆盖率。
  - 流利度：语速、停顿比例和异常长停顿。
  - 节奏：词时长与停顿位置的基础反馈。
  - 原始词级时间戳只参与本次计算，不进入自动备份；不跨厂商直接比较分数。
- [ ] **T8.7** 时延与降级：停止说话到最终结果 P95 目标 ≤3 秒；LLM 只解释结构化指标，不进入原始评分关键链路。任何云端错误都必须保留用户输入并允许切回系统语音。

**完成判据**：首启双 Key 和三项能力验证不可跳过；系统/云端模式可切换；中国大陆/海外 Profile 可独立测试；已完成首启后云端临时不可用时仍能输入和朗读；真实一句英文结束后约 3 秒内返回基础语音反馈。
...
| W5 | T8（语音双模式 + 区域 Profile + 基础反馈） | 国内外网络可切换，云端失败可降级 |
...
- 音素级发音评分、语法课程、词汇量统计（MVP 只做可懂度、完整度、流利度和节奏）
- 图谱可视化、embedding 检索
- 社交、排行榜、连续打卡天数
- MVP 后端服务（本地通知已经覆盖当前必须的服务端理由；正式版的认证、计费和模型代理见 `ENGINEERING.md`）
